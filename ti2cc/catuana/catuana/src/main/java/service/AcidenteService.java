...
/*
	...
	private final int FORM_ORDERBY_ADM     = 6;
	private final int FORM_ORDERBY_MEDICO  = 7;
	...
	public void makeForm(int tipo, Usuario usuario, int orderBy) {
		String nomeArquivo = "usuarioform.html";
		form = "";
		try{
			Scanner entrada = new Scanner(new File(nomeArquivo));
		    while(entrada.hasNext()){
		...
		if(tipo != FORM_INSERT) {
			umUsuario += "\t<table width=\"80%\" bgcolor=\"#f3f3f3\" align=\"center\">";
			umUsuario += "\t\t<tr>";
			umUsuario += "\t\t\t<td align=\"left\"><font size=\"+2\"><b>&nbsp;&nbsp;&nbsp;<a href=\"/usuario/list/1\">Novo Usuarioistrador</a></b></font></td>";
			umUsuario += "\t\t</tr>";
			umUsuario += "\t</table>";
			umAdmin += "\t<br>";			
		}
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			String action = "/usuario/";
			String nome, cpf, email, celular, senha, adm, medico, buttonLabel;
			if (tipo == FORM_INSERT){
				action += "insert";
				nome    = "Inserir Usuario";
				cpf     = "12345678910";
				...
				nome    = "Atualizar Usuario (Cpf " + usuario.getCpf() + ")" ;
				cpf     = "12345678910";
				email   = "nome@email.com";
				celular = "31999999999";
				senha   = "nome1234#";
